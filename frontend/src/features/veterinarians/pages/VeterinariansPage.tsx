...
function combineClinics(
  active: VeterinaryClinic[],
  inactive: VeterinaryClinic[],
): VeterinaryClinic[] {
  const clinics = new Map<string, VeterinaryClinic>();

  for (const clinic of [...active, ...inactive]) {
    clinics.set(clinic.id, clinic);
  }

  return [...clinics.values()].sort((first, second) =>
...
export function VeterinariansPage() {
  ...
  useEffect(() => {
    setPage(1);
  }, [statusFilter, normalizedSearch, pageSize, clinicFilter]);

  const activeClinicsQuery = useQuery({
    queryKey: ["veterinary-clinics", "options", true],
  ...
  const inactiveClinicsQuery = useQuery({
    queryKey: ["veterinary-clinics", "options", false],
  ...
  const activeClinics = activeClinicsQuery.data?.items ?? [];

  const allClinics = useMemo(
    () => combineClinics(activeClinics, inactiveClinicsQuery.data?.items ?? []),
    [activeClinics, inactiveClinicsQuery.data?.items],
  );
  ...
  const veterinariansQuery = useQuery({
    ...
    queryFn: async (): Promise<PaginatedVeterinarians> => {
      if (normalizedSearch) {
        const found = await searchVeterinarians({
          search: normalizedSearch,
          isActive,
        });

        const items = clinicFilter
          ? found.filter(
              (veterinarian) =>
                veterinarian.veterinaryClinicId === clinicFilter,
            )
          : found;
      ...
      if (clinicFilter) {
        const items = await getVeterinariansByClinic(clinicFilter, isActive);
  ...
  const usingLocalResult = Boolean(normalizedSearch) || Boolean(clinicFilter);
    ...
    <section className="space-y-6">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <button
          ...
          disabled={activeClinicsQuery.isLoading}
      ...
      <VeterinarianFormModal
        ...
        isLoadingClinics={activeClinicsQuery.isLoading}
