...
export function VeterinaryClinicsPage() {
  ...
  const [modalState, setModalState] =
    useState<ClinicModalState | null>(
      null,
    )

  const [
    selectedVeterinariansClinic,
    setSelectedVeterinariansClinic,
  ] = useState<VeterinaryClinic | null>(
    null,
  )
  ...
  const clinicsQuery = useQuery({
    ...
    queryFn:
      async (): Promise<PaginatedVeterinaryClinics> => {
        if (normalizedSearch) {
          const items =
            await searchVeterinaryClinics(
              {
                search:
                  normalizedSearch,
                isActive,
  ...
  const createMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      toast.success(
        'Veterinaria registrada correctamente.',
      )
    },
  })
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      toast.success(
        'Veterinaria actualizada correctamente.',
      )
    },
  })
  ...
  const deactivateMutation =
    useMutation({
      ...
      onSuccess: async () => {
        ...
        toast.success(
          'Veterinaria desactivada correctamente.',
        )
      },
      ...
      onError: (error) => {
        toast.error(
          getApiErrorMessage(
            error,
            'No fue posible desactivar la veterinaria.',
          ),
        )
      },
    })
  ...
  const restoreMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      toast.success(
        'Veterinaria restaurada correctamente.',
      )
    },
    ...
    onError: (error) => {
      toast.error(
        getApiErrorMessage(
          error,
          'No fue posible restaurar la veterinaria.',
        ),
      )
    },
  })
      ...
      <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-slate-500">
            Administra clínicas veterinarias,
            sus datos de contacto y sus
